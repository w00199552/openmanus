...
import {Send, Plus, Paperclip, Cpu, Square, AtSign, Sparkles, Check} from "lucide-react";
      ...
      <div className="content-narrow">
        ...
        {showSkillPicker && skills.length > 0 && (
          <div className="absolute bottom-full left-2 right-2 mb-1 max-h-60 overflow-y-auto rounded-lg border border-border/60 bg-popover shadow-lg">
            <div className="px-3 py-2 text-[11px] font-medium text-muted-foreground/60">
              <Sparkles className="mr-1 inline size-3"/> Select a skill
            </div>
            {skills.map((skill, i) => (
              <button
                key={skill.name}
                ...
                className={cn(
                  ...
                  i === skillIndex ? "bg-accent/10" : "hover:bg-sidebar/40",
                )}
              >
                <Sparkles className={cn("size-3.5 shrink-0", i === skillIndex ? "text-accent" : "text-muted-foreground/50")}/>
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] font-medium">{skill.name}</div>
                  <div className="truncate text-[11px] text-muted-foreground/60">{skill.description}</div>
                </div>
                {i === skillIndex && <Check className="size-3 shrink-0 text-accent"/>}
              </button>
            ))}
          </div>
        )}
        ...
        <div className="rounded-xl border border-border/60 bg-card px-2.5 py-2 transition focus-within:border-accent/40">
          ...
          <div className="mt-1.5 flex items-center gap-1">
            ...
            <ToolBtn
              label="Skills"
              onClick={() => { setValue("/skill "); taRef.current?.focus(); }}
            >
